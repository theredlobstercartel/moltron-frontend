...
interface ProjectConfig {
    name: string;
    type: 'component' | 'page' | 'app' | 'dashboard';
    aesthetic: 'cyber-neon' | 'brutalist' | 'organic' | 'retro-future' | 'editorial' | 'minimal-luxury';
...
async function checkShadcn(): Promise<boolean> {
    try {
...
async function initProject(config: ProjectConfig): Promise<{ success: boolean; error?: string }> {
    ...
    try {
        ...
        await execAsync(
            `npx create-next-app@latest my-app --typescript --tailwind --eslint --app --src-dir=false --import-alias="@/*" --use-npm --no-turbopack`,
            { cwd: projectDir }
        );
        ...
        await execAsync(
            `echo "my-app" | npx shadcn@latest init --yes --base-color ${config.theme === 'dark' ? 'neutral' : 'stone'}`,
            { cwd: projectDir }
        );
        ...
        await execAsync(
            'npm install framer-motion@latest next-seo@latest lucide-react@latest clsx@latest tailwind-merge@latest'
        );
...
function generateAestheticCSS(aesthetic: ProjectConfig['aesthetic'], theme: 'dark' | 'light'): string {
    const aesthetics = {
        'cyber-neon': {
            dark: `
:root {
...
function generateSEOConfig(projectName: string, description: string): object {
    return {
...
async function main() {
    ...
    if (command === 'init') {
        ...
        const aesthetic = (args.find(a => a.startsWith('--aesthetic='))?.split('=')[1] || 'cyber-neon') as ProjectConfig['aesthetic'];
